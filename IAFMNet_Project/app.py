...
def build_ui(model_path: str):
    ...
    with gr.Blocks(
        title="IAFMNet Super-Resolution"
    ) as demo:

        gr.Markdown(
            "# 🖼️ IAFMNet Super-Resolution"
        )

        gr.Markdown(
            "Upload a low-resolution image → "
            "get 4× super-resolved output + "
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                out_sr = gr.Image(
                    label="Output (Super-Resolved ×4)",
                    type="numpy",
                )

                out_idm = gr.Image(
                    label="Information Density Map",
                    type="numpy",
                )
        ...
        run_btn.click(
            fn=lambda img: process(
                img,
                model_path,
            ),
            inputs=inp,
            outputs=[out_sr, out_idm],
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="IAFMNet Super-Resolution Web App."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=7860,
        help="Port for the Gradio application.",
    )
    ...
    if not Path(args.model).exists():
        ...
        print(
            "Please make sure the trained ONNX model exists at:"
        )
        ...
        print(
            "If the ONNX model has not been created yet, run:"
        )
